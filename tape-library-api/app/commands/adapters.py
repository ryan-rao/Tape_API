...
class MtxAdapter:
    ...
    def exchange(self, changer: str, source: int, destination: int):
        return ["mtx", "-f", normalize_device(changer), "exchange",
                str(validate_slot(source)), str(validate_slot(destination))]

    def first(self, changer: str):
        return ["mtx", "-f", normalize_device(changer), "first"]

    def next(self, changer: str):
        return ["mtx", "-f", normalize_device(changer), "next"]

    def last(self, changer: str):
        return ["mtx", "-f", normalize_device(changer), "last"]
