...
def parse_sg_inq(stdout):
    ...
    parsed = {k: fields[k] for k in wanted if k in fields}
    parsed["parsed_ok"] = bool(parsed)
    return parsed
...
def parse_mtx_status(stdout):
    ...
    for line in stdout.splitlines():
        s = line.strip()
        m = re.match(r"Storage Element (\d+):(Full|Empty)", s)
        if m:
            slot = {"element": int(m.group(1)), "occupied": m.group(2) == "Full",
                    "import_export": "Import/Export" in s, "barcode": None}
...
def parse_sg_vpd(stdout):
    ...
    parsed["parsed_ok"] = parsed["page_code"] is not None or bool(parsed["fields"])
    return parsed
...
def parse_sg_modes(stdout):
    ...
    pages = []
    for line in stdout.splitlines():
        m = re.search(r"([A-Za-z][A-Za-z0-9 _/-]*?)\s+page\s*\(?0x([0-9a-fA-F]+)\)?", line)
        if m:
            pages.append({"page_name": m.group(1).strip(), "page_code": "0x" + m.group(2).lower()})
    fields = {}
    for line in stdout.splitlines():
...
def parse_sg_logs(stdout, page=None):
    """sg_logs [-a|-p page] output: log page headers + params + hex dumps."""
    pages = []
    for line in stdout.splitlines():
        m = re.search(r"([A-Za-z][A-Za-z0-9 _/-]*?)\s+(?:log sense )?page\s*\(?0x([0-9a-fA-F]+)\)?", line)
        if m:
            pages.append({"page_name": m.group(1).strip(), "page_code": "0x" + m.group(2).lower()})
    fields = {}
    for line in stdout.splitlines():
        m = re.match(r"\s*([A-Za-z0-9 _/]+?):\s*(\d+(?:\.\d+)?)\s*$", line)
        if m:
            fields[m.group(1).strip().lower().replace(" ", "_")] = float(m.group(2)) \
                if "." in m.group(2) else int(m.group(2))
    parsed = {"requested_page": page or "all", "log_pages": pages, "parameters": fields}
    parsed.update(_hex_dump_stats(stdout))
    parsed["parsed_ok"] = bool(pages) or bool(fields)
    return parsed
